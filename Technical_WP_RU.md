...
interface IExternalPayment {
    function paymentReceived(address from, uint amount) external;
}

contract Action {
    function process(bytes S) external {
        require(sha256(S) == hash);
        target.paymentReceived(AliceAddress, paymentAmount);
    }
...
Общая схема работы:
1. Арендодатель деплоит контракт, указывая:
  - количество дней, на которое сдается помещение
  - описание помещения
  - цену в эфирах за весь срок аренды
  - время до начала периода аренды, после которого клиент возврат средств клиент может произвести только со штрафом
  - время после начала аренды, которое дается клиенту на приемку объекта
2. Клиент, отправляет эфир на контракт аренды
3. До начала периода аренды:
  - Арендодатель проверяет платеж и, при необходимости, может его возвратить
  - Клиент может сделать возврат. При этом из возвращаемых средств может быть удержан штраф, если клиент возвращает средства после указанного Арендодатором периода до начала аренды
4. После начала периода аренды:
  - Клиент в отведенное время для приемки должен принять помещение, и сообщить об этом, вызвав специальный метод контракта. После этого деньги за аренду уходят из смарт-контракта Арендодаторую
  - Если Клиента помещение не устраивает, то он может сделать возврат средств, уплатив при этом штраф.
  - В случае, если Клиент не принимает объект в отведенное для этого время, Арендодатор может отменить аренду. При этом уплаченные Клиентом средства за вычетом штрафа возвращаются Клиенту.
...
2. Участники отправляют токены и/или эфир на задеплоенный контракт
3. Участники в любой момент до начала обмена могут вернуть свои средства, вызвав специальный метод контракта
4. После того, как оба участника отправлили средства на контракт, вызвав метод для начала обмена, контракт отправляет токены и/или эфир на указанные адреса

В качестве особенностей конструктора стоит отметить сложную JSON-схему для описание параметров контракта. В зависимости от выбора опций (использовать ли адрес человека, который деплоит, в качестве адреса одного из участников, менять токены на эфир или токены) интерфейс для ввода параметров, интерфейс деплоя контракта меняется, чтобы показывать только нужные поля.
