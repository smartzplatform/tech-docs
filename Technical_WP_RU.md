...
В контракте может существовать несколько ролей, имеющих различные возможности взаимодействия с контрактом. Напр., в 
контракте аренды есть арендодатель и арендатор. Контракты смогут перечислить адреса, связанные с той или иной ролью
с помощью публичных константных функций (их вызов не требует отправки транзакций, напр., в Ethereum это функции, 
помеченные как `public view` или `external view`). Разработчик сможет разметить каждую из функций контракта ролями,
для которых имеет смысл вызов данной функции. Smartz автоматически определит список ролей текущего пользователя (зная
его блокчейн-адреса) и сформирует подмножество функций, доступных ему.

Нужно, однако, помнить, что это делается исключительно для облегчения инерфейса и улучшения пользовательского опыта.
Разграничения доступа должны выполняться только средствами блокчейна.
...
Хэши версий приложений будут фиксироваться в блокчейнах (anchoring) в контрактах-реестрах (см. Контракты), т.о. даже
в случае компрометации платформы делается невозможным незаметная подмена существующих на данный момент DApps.
...
**Roles**:
Плательщик, Получатель, Escrow

**Description**:
Одноразовый контракт, при создании принимает некоторую сумму от Плательщика. Затем Escrow принимает решение отдать ли сумму Плательщику, либо не согласиться и вернуть средства Плательщику. В случае истечения срока принятия решения средства могут быть возвращены Плательщиком.

**User story**:
- Плательщик деплоит контракт escrow:
  - указав адрес Получателя, адрес Escrow, max время принятия решения
  - положив на адрес контракта всю сумму, относительно которой будет приниматься решение
- Контракт, с размещенной суммой, переходит в состояние wait-for-escrow, ожидая транзакцию от Escкow
- Если транзакция от Escrow приходит до истечения TTL контракта:
  - контракт отправляет весь баланс Получателю или Плательщику, в зависимости от решения Escrow
- Во всех остальных случаях Плательщик может вернуть свои средства, отправив транзакцию в контракт

**Notes**:
Как и для всех одноразовых контрактов, для эффективного использования state database блокчейна разумно выполнять selfdestruct() в любой транзакции, завершающей работу контракта.
...
**Roles**:
Пользователь, Получатель(опционально)

**Description**:
одноразовый контракт, при создании принимает некоторую сумму от Плательщика. Затем, разрешает ее забрать Получателю лишь по истечении указанного времени (freeze-ttl). После истечения срока хранения Получатель может забрать всю сумму. 

**User story**:
- Плательщик деплоит контракт freeze:
  - указав адрес Получателя (либо свой собственный в качестве Получателя)
  - указав время, на которое средства должны быть заморожены
 - Если транзакция от Получателя приходит до истечения freeze-ttl, она либо пополняет баланс контракта, либо откатывается
 - Если транзакция от Получателя приходит после истечения freeze-ttl, контракт отправляет весь баланс Получателю

**Notes**:
Указав свой адрес в качестве Получателя, Плательщик получит вариант с "копилкой". В случае внешнего Получаетеля, контракт позволяет сделать гарантированный, но отложенный платеж. Как и для всех одноразовых контрактов, для эффективного использования state database блокчейна разумно выполнять selfdestruct() в любой транзакции, завершающей работу контракта.
