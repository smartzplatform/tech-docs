...
Общая схема работы: Арендодатель деплоит контракт с условиями аренды, Клиент присылает эфир на контракт. После того, как Клиент проверит помещение, эфир за аренду уходит Арендодателю.
...
Конструктор контрактов реестра - пример нешаблонного конструктора, полностью настраивающего итоговый контракт. Имена полей и имена функций поиска по ним формируются на основе названия этих полей формируются на основе названий этих полей, которые ввел владелец реестра.
